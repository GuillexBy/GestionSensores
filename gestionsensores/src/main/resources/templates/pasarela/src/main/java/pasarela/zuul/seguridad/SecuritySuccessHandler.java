...
public class SecuritySuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (datosUsuario != null) {
			...
			Cookie rol = new Cookie("rol", datosUsuario.get("rol"));
			rol.setMaxAge(JwtUtils.EXPIRATION_TIME); 
			// cookie.setSecure(true);
			rol.setHttpOnly(true);
			rol.setPath("/");
			response.addCookie(rol);
			
			Cookie email = new Cookie("email", datosUsuario.get("email"));
			email.setMaxAge(JwtUtils.EXPIRATION_TIME); 
			// cookie.setSecure(true);
			email.setHttpOnly(true);
			email.setPath("/");
			response.addCookie(email);
			System.out.println("Cookies insertadas");
			...
			redirectStrategy.sendRedirect(request, response, "http://localhost:3000/logged");//hay que tocar esto
